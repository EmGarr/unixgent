...
| ~~Hard iteration cap + no compaction~~ | ~~High~~ | FIXED — Removed `MAX_AGENT_ITERATIONS` in REPL (unbounded, continues until model stops). Batch mode iteration cap removed entirely (was 50, now unbounded loop — parent agent controls budget via system prompt). Token-aware compaction via `compact_conversation()` triggers when `input_tokens > 100k`. Summarizes old turns via non-streaming LLM call, preserves recent 4 turns. API overflow errors trigger compaction + retry. |
