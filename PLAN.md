...
| RLM-aligned system prompt patterns | 2026-02-17 | System prompt updated to explicitly teach 6 RLM patterns that were implicit or missing: (1) **Context-scale triggers** — delegate when data exceeds context window, not just on step count; (2) **File-as-environment** — filesystem is working memory with no size limit, write intermediate results to files; (3) **Peek-before-read** — inspect structure (wc -l, head, find) before committing to full reads; (4) **Chunk-map-reduce** — partition large data, delegate each chunk to subagent, aggregate results; (5) **No lossy compression** — pass file paths between agents, never summarize large outputs into conversation; (6) **Instruction richness** — subagent instructions must include specific questions, file paths, and expected output formats. Base prompt gains CONTEXT MANAGEMENT section. DELEGATION section restructured into WHEN TO DELEGATE, DECOMPOSITION STRATEGIES (fan-out, chunk-map-reduce, peek-then-delegate), WRITING EFFECTIVE SUBAGENT INSTRUCTIONS, and RESULT AGGREGATION. |
