...
| Agent state management redesign | 2026-02-27 | Introduced `PendingCommands` struct (commands + tool_use_ids + risk_levels + has_privileged + use_cr_reset) to eliminate field threading through Judging → Approving → Executing transitions. `AgentTurn` struct centralizes turn-level accumulators (iteration, tokens, conversation cache). Removed `iteration` from `AgentState` variants (lives in `AgentTurn`). `CommandAction` now carries `PendingCommands` instead of 6 individual fields. `flush_pty_buffer()` helper replaces 6 copy-paste occurrences. `make_capture()` replaces repeated if/else for OutputHistory construction. `OscParser.terminal_state` made private with accessor method. `start_streaming` takes `&mut AgentTurn` instead of 3 separate mutable refs. `start_judging` takes `PendingCommands` instead of 6 params. Eliminated `pending_instruction` (instruction lives in journal). `turn = None` replaces 4-line token counter reset pattern in all cancel/deny paths. Landlock `add_rule` / `restrict_self` fixed for landlock 0.4 API (needs `.create()` before rules). |
